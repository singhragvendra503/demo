...
for ((i=0; i<$array_length; i++)); do
 ...
 #upto 199 unit, per unit charge is @1.20
  if [ "$value" -le 199 ]; then
    ...
    json_data=$(echo "$json_data" | jq --argjson i "$i" --arg updated_bill $updated_bill '. as $data | .[$i] |= . + {"Total Bill": $updated_bill}')
...
echo "$json_data" >MOCK_DATA.json 
}

function show_details()
{
# Store the JSON file contents in a variable
json=$(cat MOCK_DATA.json)
